...
def apply_approved_rebalance_plan(
    ...
    *,
    ...
    existing_event = _event_for_plan(root, str(plan.get("plan_id")))
    if plan.get("status") == APPLIED_STATUS:
        return {
            "applied": False,
    ...
    event = {
        ...
        "event_id": f"applied-rebalance-{uuid4().hex[:12]}",
    ...
    _write_event_store(root, events)
